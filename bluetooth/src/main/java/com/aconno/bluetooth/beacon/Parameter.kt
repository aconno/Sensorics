...
class Parameter(
    ...
    val type: ValueConverter = when ((ValueConverter.UINT16.converter.deserialize(data.copyOfRange(0, 2)) as Int)) {
    ...
    var value: Any = when (type) {
        ...
        ValueConverter.UTF8STRING ->
            type.converter.deserialize(data.copyOfRange(24, 24 + data.stringLength(24)), order = ByteOrder.BIG_ENDIAN)!!
        else -> {
            type.converter.deserialize(data.copyOfRange(24, 28))!!
        }
    }
        set(value) {
    ...
    fun write(): Task {
        return ParameterNameWriteTask()
    }

    inner class ParameterNameWriteTask : CharacteristicWriteTask(characteristicUUID = PARAMETER_INDEX_UUID, value = byteArrayOf(id.toByte())) {
        override fun onSuccess() {
            Timber.e("About to write parameter: $name")
            taskQueue.offer(ParameterDataWriteTask())
        }
    ...
    inner class ParameterDataWriteTask : CharacteristicWriteTask(
            characteristicUUID = PARAMETER_DATA_UUID,
            value = type.converter.serialize(this@Parameter.value.toString()).copyOf(56)
    ) {
        override fun onSuccess() {
