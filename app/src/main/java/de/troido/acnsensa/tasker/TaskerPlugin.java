...
public class TaskerPlugin {
    ...
    /**
     * Allows the plugin/host to indicate to each other a set of variables which they are
     * referencing.
     * The host may use this to e.g. show a variable selection list in it's UI.
     * The host should use this if it previously indicated to the plugin that it supports
     * relevant vars
     *
    ...
    public static String[] getRelevantVariableList(Bundle fromHostIntentExtras) {
        ...
        String[] relevantVars = (String[]) getBundleValueSafe(fromHostIntentExtras,
                BUNDLE_KEY_RELEVANT_VARIABLES, String[].class, "getRelevantVariableList");
     ...
     *
     * @param resultExtras the result extras from the receiver onReceive (from a call to
     *                     getResultExtras())
     ...
     * <p>
     * This is completely optional and currently only necessary if using
     * Setting#setVariableReplaceKeys
    ...
    public static class Setting {
         ...
         * <p>
         * Note that the user needs to have configured the task to continue after failure of the
         * plugin
         * action otherwise they will not be able to make use of the error message.
         * <p>
         * For use with #addRelevantVariableList(Intent, String[]) and #addVariableBundle(Bundle,
         * Bundle)
         */
         ...
         * @see #setVariableReplaceKeys(Bundle, String[])
         */
        private final static String BUNDLE_KEY_VARIABLE_REPLACE_STRINGS = EXTRAS_PREFIX +
        ...
        public final static int REQUESTED_TIMEOUT_MS_NEVER = REQUESTED_TIMEOUT_MS_MAX +
                1000;
         ...
         */
        private final static String EXTRA_PLUGIN_COMPLETION_INTENT = EXTRAS_PREFIX +
                "COMPLETION_INTENT";
        ...
        public static boolean hostSupportsOnFireVariableReplacement(Bundle extrasFromHost) {
            return hostSupports(extrasFromHost,
                    EXTRA_HOST_CAPABILITY_SETTING_FIRE_VARIABLE_REPLACEMENT);
        }
        ...
        public static boolean hostSupportsOnFireVariableReplacement(Activity editActivity) {
            ...
            boolean supportedFlag = hostSupportsOnFireVariableReplacement(editActivity.getIntent
                    ().getExtras());
            ...
            if (!supportedFlag) {
                ...
                if (callingActivity == null)
                    Log.w(TAG, "hostSupportsOnFireVariableReplacement: null callingActivity, " +
                            "defaulting to false");
                else {
                    ...
                    supportedFlag =
                            (callerPackage.startsWith(BASE_KEY)) &&
                                    (getPackageVersionCode(editActivity.getPackageManager(),
        ...
        public static void requestTimeoutMS(Intent intentToHost, int timeoutMS) {
            if (timeoutMS < 0)
                ...
                if (
                    ...
                    Log.w(TAG, "requestTimeoutMS: requested timeout " + timeoutMS + " exceeds " +
                            "maximum, setting to max (" + REQUESTED_TIMEOUT_MS_MAX + ")");
                    timeoutMS = REQUESTED_TIMEOUT_MS_MAX;
                }
         ...
         * @see #setKeyEncoding(Bundle, String[], Encoding)
         */
        public static void setVariableReplaceKeys(Bundle resultBundleToHost, String[]
                listOfKeyNames) {
            addStringArrayToBundleAsString(
         ...
         */
        public static boolean signalFinish(Context context, Intent originalFireIntent, int
                resultCode, Bundle vars) {

            String errorPrefix = "signalFinish: ";

            boolean okFlag = false;

            String completionIntentString = (String) getExtraValueSafe(originalFireIntent,
                    Setting.EXTRA_PLUGIN_COMPLETION_INTENT, String.class, "signalFinish");
            ...
            if (completionIntentString != null) {
                ...
                if (completionIntentUri != null) {
                    try {
                        Intent completionIntent = Intent.parseUri(completionIntentString, Intent
                                .URI_INTENT_SCHEME);
        ...
        public static int getHintTimeoutMS(Bundle extrasFromHost) {
            ...
            Bundle hintsBundle = (Bundle) TaskerPlugin.getBundleValueSafe(extrasFromHost,
                    EXTRA_HINTS_BUNDLE, Bundle.class, "getHintTimeoutMS");

            if (hintsBundle != null) {

                Integer val = (Integer) getBundleValueSafe(hintsBundle,
                        BUNDLE_KEY_HINT_TIMEOUT_MS, Integer.class, "getHintTimeoutMS");
    ...
    public static class Event {
        ...
        private final static String EXTRA_REQUEST_QUERY_PASS_THROUGH_DATA = EXTRAS_PREFIX +
                "PASS_THROUGH_DATA";
        ...
        /**
         * Specify a bundle of data (probably representing whatever change happened in the
         * condition)
         ...
         * addPassThroughMesssageID() can be used to add an ID if the plugin doesn't wish to add
         * it's
         ...
         * <p>
         * Note that if addPassThroughMessageID() was previously called, the data will contain an
         * extra
        ...
        /**
         * Add a message ID to a REQUEST_QUERY intent which will then be included in the
         * corresponding
         ...
         *
         * @param requestQueryIntent intent being sent to the host
         * @return an ID for the bundle so it can be identified and the caller verified when it
         * is again received by the plugin
         * @see #hostSupportsRequestQueryDataPassThrough(Bundle)
         * @see #retrievePassThroughData(Intent)
         */
        ...
        */
        ...
        private static Bundle retrieveOrCreatePassThroughBundle(Intent requestQueryIntent) {
            ...
            if (requestQueryIntent.hasExtra(EXTRA_REQUEST_QUERY_PASS_THROUGH_DATA))
                passThroughBundle = requestQueryIntent.getBundleExtra
                        (EXTRA_REQUEST_QUERY_PASS_THROUGH_DATA);
            else {
                passThroughBundle = new Bundle();
                requestQueryIntent.putExtra(EXTRA_REQUEST_QUERY_PASS_THROUGH_DATA,
                        passThroughBundle);
            }
    ...
    public static class Host {
        ...
        /**
         * Add an intent to the fire intent before it goes to the plugin FireReceiver, which the
         * plugin
         * can use to signal when it is finished. Only use if
         * @code{pluginWantsSychronousExecution} is true.
         *
        ...
        public static int getSettingResultCode(Intent completionIntent) {
            ...
            Integer val = (Integer) getExtraValueSafe(completionIntent, Setting
                    .EXTRA_RESULT_CODE, Integer.class, "getSettingResultCode");
        ...
        public static void addHintTimeoutMS(Intent toPlugin, int timeoutMS) {
            getHintsBundle(toPlugin, "addHintTimeoutMS").putInt(BUNDLE_KEY_HINT_TIMEOUT_MS,
                    timeoutMS);
        }

        private static Bundle getHintsBundle(Intent intent, String funcName) {

            Bundle hintsBundle = (Bundle) getExtraValueSafe(intent, EXTRA_HINTS_BUNDLE, Bundle
                    .class, funcName);
        ...
        public static int getRequestedTimeoutMS(Bundle extrasFromPluginEditActivity) {
            return
                    (Integer) getBundleValueSafe(
                            extrasFromPluginEditActivity, Setting.EXTRA_REQUESTED_TIMEOUT,
                            Integer.class, "getRequestedTimeout"
                    )
                    ;
        }
        ...
        public static String[] getKeysWithEncoding(Bundle fromPluginEditActivity, Encoding
                encoding) {
    ...
    private static Object getBundleValueSafe(Bundle b, String key, Class<?> expectedClass, String
            funcName) {
        Object value = null;
    ...
    private static Object getExtraValueSafe(Intent i, String key, Class<?> expectedClass, String
            funcName) {
    ...
    private static String[] getStringArrayFromBundleString(Bundle bundle, String key, String
            funcName) {
