...
class SavedDevicesDialog : DialogFragment(), ItemClickListener<Device> {

    @Inject
    lateinit var savedDevices: Flowable<List<Device>>

    private var disposable: Disposable? = null
    private lateinit var adapter: DeviceAdapter

    private lateinit var listener: SavedDevicesDialogListener
    ...
    override fun onAttach(context: Context?) {
        ...
        if (activity is SavedDevicesDialogListener) {
            listener = activity
        } else {
            throw RuntimeException("$activity must implement ${SavedDevicesDialogListener::class}")
        }
    }
    ...
    override fun onItemClick(item: Device) {
        listener.onSavedDevicesDialogItemClick(item)
        dialog.dismiss()
    }
